...
```{r my_boxplot}
#| echo: false
my_boxplot <- ggplot(penguins_sm, aes(fill = species)) +
...
- **Before Stat** validates the layer's choice of _stat_

- **After Stat** context for `after_stat()` mappings

- **Before Geom** validates the layer's choice of _geom_

- **After Scale** context for `after_scale()` mappings

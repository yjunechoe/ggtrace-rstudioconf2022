...
[outline]{.fragment fragment-index=2}

:::: {.columns}

::: {.column .fragment fragment-index=2 width="60%"}

1) Reframing of ggplot internals as data wrangling

2) Demo using a simple barplot example
...
```{r layer2}
#| eval: false
#| attr-source: 'style="background-color:white" data-hl-loc="[[5.8,4.8,5,\"#FFB1C0\"],[7.2,4.8,19.3,\"#FFB1C0\"],[1.45,5.3,4,\"#FFB1C0\"],[2.9,8.6,11.9,\"#FFB1C0\"],[2.9,21,9.5,\"#E1C563\"],[4.3,7.5,8.6,\"#E1C563\"],[0,2.8,2.8,\"#64DF91\"],[5.8,10.7,4.5,\"#E0B6FF\"]]"'
```
</div>
:::
...
```{r arrows, results="asis"}
#| echo: false
div(lapply(1:4, function(i) {
  div(class = c("absolute", "step-arrow"), "⟵",
      style = paste0("top:", c(200,270,300,450)[i], "px; left:", c(500,500,440,440)[i],
                     "px; color:", c("#E16A86", "#AA9000", "#00AA5A", "#B675E0")[i], ";"))
}))
```
...
::: {.column width="35%"}
```{r snapshot-blocks-hl}
#| echo: false
#| results: "asis"
div(class = "internal-steps-container", lapply(1:4, function(i) {
  div(class = "internal-step",
      style = paste0("background-color:#", c("BFBFBF", "E4CB73", "BFBFBF", "BFBFBF")[i]),
      c("Before Stat", "After Stat", "Before Geom", "After Scale")[i])
}))
```
:::
...
```{r arrows-hl, results="asis"}
#| echo: false
div(lapply(1:4, function(i) {
  div(class = c("absolute", "step-arrow"), "⟵",
      style = paste0("top:", c(200,270,300,450)[i], "px; left:", c(500,500,440,440)[i],
                     "px; color:", c("#BFBFBF", "#AA9000", "#BFBFBF", "#BFBFBF")[i], ";"))
}))
```
...
![](https://allisonhorst.github.io/palmerpenguins/reference/figures/culmen_depth.png){.absolute bottom=50 right=0 width=300}
...
- [After Stat]{.my-hl-2} [has `count`, allows in `y = after_stat(count)`]{.fragment}

- [Before Geom]{.my-hl-3} [has `x` and `y`, validates `"bar"` geom]{.fragment}

- [After Scale]{.my-hl-4} [allows `after_scale()` mappings (unused)]{.fragment}
...
1) Every aesthetic mapping is like a `mutate()` scheduled for different `stage()`s of the internal pipeline.

. . .

2) Every layer has a `Stat` and a `Geom` - they steps in to transform the data as long as the required columns are present.
